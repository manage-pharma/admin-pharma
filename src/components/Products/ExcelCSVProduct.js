...
const ExcelCSVProductComponent = () => {
        ...
        if (sheets.length) {
          ...
          for (const item of cloneData) {
            ...
            if (!d.errors.length) {
              ...
              item.categoryDrug =
                item?.categoryDrug && JSON.parse(item?.categoryDrug);
              item.APIs = item?.APIs?.length > 0 ? JSON.parse(item?.APIs) : [];
              item.image =
                item?.image?.length > 0 ? item?.image?.split(";") : [];
              dataNew.push(item);
            } else {
              rowErrors.push({
  ...
  const handleExport = () => {
    ...
    cloneData.map((item) => {
      ...
      item.categoryDrug =
        item?.categoryDrug && JSON.stringify(item?.categoryDrug);
          ...
          <header className="card-header bg-white ">
            <div className="row gx-3 py-3">
              ...
              {errors?.length ? (
                ...
                <DataTableProduct products={data} unShowSetting={true} />
              ) : (
