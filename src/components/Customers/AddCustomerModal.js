...
const AddCustomer = (props) => {
  ...
  const { success: successCustomerSingle, customer: customerEdit } =
    customerEditing;
  const customerID = customerEdit._id;

  const customerUpdated = useSelector((state) => state.customerUpdate);
  const { error: errorUpdate, success: successCustomerUpdated } =
    customerUpdated;
  ...
  useEffect(() => {
    ...
    dispatch,
    setShow,
    successCustomerSingle,
    successCustomerUpdated,
    customerEdit,
    errorCreate,
    errorUpdate,
  ]);
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title id="contained-modal-title-vcenter">
            {successCustomerSingle ? `Cập nhật ${name}` : "Thêm người dùng"}
          </Modal.Title>
        </Modal.Header>
