...
import {
  ...
  PROVIDER_SINGLE_RESET,
  PROVIDER_UPDATE_RESET,
} from "../../Redux/Constants/ProviderConstants";
const ToastObjects = {
...
const AddProvider = (props) => {
  ...
  const { success: successProviderSingle, provider: providerEdit } =
    providerEditing;
  const providerID = providerEdit._id;

  const providerUpdated = useSelector((state) => state.providerUpdate);
  const { error: errorUpdate, success: successProviderUpdated } =
    providerUpdated;
