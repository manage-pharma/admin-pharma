...
} from "../../Redux/Constants/ImportStockConstant";
...
const EditImportStock = (props) => {
  ...
  const { success, error } = importUpdate;
  ...
  totalVAT = itemProducts.reduce(
    (sum, curr) =>
      sum +
      +curr.price * +curr.qty * (1 - +curr.discount / 100) * (+curr.VAT / 100),
    0,
  );
  totalDiscount = itemProducts.reduce(
  ...
  const handleAddProduct = (e) => {
    ...
    if (!field.product) {
      ...
      +moment(field.expDrug).diff(moment(field.manufactureDate), "months") <
      +field.expProduct
    ) {
      renderToast(
        `Hạn sử dụng của thuốc phải lớn hơn ${+field.expProduct} tháng `,
        "error",
        setIsStop,
        isStop,
      );
    ...
    itemProducts.forEach((item, index) => {
      if (
        (item.product._id || item.product) === field.product &&
  ...
  return (
    <>
      ...
      <section
        className={`content-main ${importStockItem?.status ? "disabled" : ""}`}
      >
              ...
              {!importStockItem?.status ? (
                ...
                <h4>
                  <span className="badge bg-danger text-white">
                    Hóa đơn này đã hoàn tất, bạn không thể chỉnh sửa
                  </span>
                </h4>
              )}
            </div>
