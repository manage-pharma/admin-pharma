...
import {
  ...
  ORDER_LIST_REQUEST,
  ORDER_LIST_SUCCESS,
  ORDER_SEARCH_LIST_FAIL,
  ORDER_SEARCH_LIST_REQUEST,
  ORDER_SEARCH_LIST_SUCCESS,
  ORDER_SEARCH_D2D_LIST_FAIL,
  ORDER_SEARCH_D2D_LIST_REQUEST,
  ORDER_SEARCH_D2D_LIST_SUCCESS,
  ORDER_CANCELED_FAIL,
  ...
  ORDER_CONFORM_FAIL,
  ORDER_CONFORM_SUCCESS,
  ORDER_CONFORM_REQUEST,
  ...
  async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.get(
        `/api/orders/all-check?from=${from}&to=${to}`,
        config,
      );
...
//ORDER LIST
export const searchD2DListOrder =
  (keyword = "", from = " ", to = " ") =>
  async (dispatch, getState) => {
    try {
      ...
      const { data } = await axios.get(
        `/api/orders/all-search?keyword=${keyword}&from=${from}&to=${to}`,
        config,
      );
...
export const getOrderConform = (orderItems) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(
      `/api/orders/${orderItems._id}/conform`,
      config,
    );
  ...
  try {
    ...
    const check = async () => {
      orderItems.orderItems.map(async (item) => {
        const { data } = await axios.get(
          `/api/drugstore/${item.drugstoreId}/check-stock?num=${item.qty}`,
          config,
        );
    ...
    const update = async () => {
      if (checkStock) {
        ...
        const { data } = await axios.put(
          `/api/orders/${orderItems._id}/delivered`,
          {},
          config,
        );
        ...
        orderItems.orderItems.map(async (item) => {
          const { data } = await axios.get(
            `/api/drugstore/${item.drugstoreId}/update-stock?num=${item.qty}`,
            config,
          ); //lấy kq trả về
        ...
        setTimeout(async () => {
          await axios.put(
            `/api/orders/${orderItems._id}/update-order-item`,
            newOrderItems,
            config,
          );
        }, 1000);
    ...
    //await upadteCoin()
    const { data } = await axios.get(
      `/api/orders/${orderItems._id}/deliverd`,
      config,
    );
    dispatch({ type: ORDER_DELIVERED_SUCCESS, payload: data });
  } catch (error) {
...
export const getOrderCanceled = (orderItems) => async (dispatch, getState) => {
  try {
    ...
    orderItems.orderItems.map(async (item) => {
      await axios.get(
        `/api/drugstore/${item.drugstoreId}/update-stock?num=${item.qty * -1}`,
        config,
      );
    });
    ...
    // api not transmiss any params because it just change state of deliverd
    const { data } = await axios.get(
      `/api/orders/${orderItems._id}/AdminCanceled`,
      config,
    );
...
export const getOrderCompleted = (orderItems) => async (dispatch, getState) => {
  try {
    ...
    // api not transmiss any params because it just change state of deliverd
    const { data } = await axios.get(
      `/api/orders/${orderItems._id}/complete`,
      config,
    );
...
export const getOrderReceived = (orderItems) => async (dispatch, getState) => {
  try {
    ...
    // api not transmiss any params because it just change state of deliverd
    const { data } = await axios.get(
      `/api/orders/${orderItems._id}/received`,
      config,
    );
