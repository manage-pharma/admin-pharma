...
import {
  ...
  PROMOTION_LIST_RESET,
  PROMOTION_CREATE_RESET,
  PROMOTION_UPDATE_RESET,
  PROMOTION_DELETE_RESET,
  PROMOTION_PRODUCT_LIST_REQUEST,
  PROMOTION_PRODUCT_LIST_SUCCESS,
  PROMOTION_PRODUCT_LIST_FAIL,
  PROMOTION_PRODUCT_LIST_RESET,
...
export const updatePromotion =
  ({ name, discount, startOn, endOn, promotionId }) =>
  async (dispatch, getState) => {
    try {
...
export const promotionProductList =
  (discountDitail) => async (dispatch, getState) => {
    try {
      dispatch({ type: PROMOTION_PRODUCT_LIST_REQUEST });

      const { data } = await axios.post(`/api/promotion/check`, {
        discountDitail,
      });
